...
def matmul_kernel5():
    ...
    with hidet.lang.script_module() as script_module:
        @hidet.lang.script
        def micro_kernel_10x8(a_ptr: ~float32,
                             b_ptr: ~float32,
                             c_ptr: ~float32,
                             pb: int32,
                             m_size: int32,
            ...
            c8 = avx_f32x8_load(~c[8, 0])
            c9 = avx_f32x8_load(~c[9, 0])
            ...
            for pp in range(pb):
                ...
                aa = avx_f32x8_broadcast(~a[8, pp])
                c8 = avx_f32x8_fmadd(aa, bb_0to7, c8)
                aa = avx_f32x8_broadcast(~a[9, pp])
                c9 = avx_f32x8_fmadd(aa, bb_0to7, c9)
            ...
            avx_f32x8_store(~c[8, 0], c8)
            avx_f32x8_store(~c[9, 0], c9)
        ...
        def macro_kernel(
            ...
            for mpanel in range(mpanels):
                ...
                for npanel in range(npanels):
                    ...
                    # micro-kernel
                    if mr == MR and nr == NR:
                        micro_kernel_10x8(~a[ii, 0], ~b[0, jj], ~c[ii, jj],
                                         pb, m_size, n_size)
                    else:
                        temp_c = tensor(
                        ...
                        micro_kernel_10x8(~a[ii, 0], ~b[0, jj], temp_c,
                                         pb, MR, NR)
