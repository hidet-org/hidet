...
def conv2d(batch_size, in_channels, height, width, out_channels, kernel, padding, stride):
    ...
    padded = compute(
        ...
        fcompute=lambda n, c, h, w: input.protect_read(indices=[n, c, h, w], default_value=0.0))
    ...
    output = compute(
        ...
        fcompute=lambda n, c, h, w: reduce(
            shape=[out_channels, kernel[0], kernel[1]],
            fcompute=lambda co, xx, yy: padded[n, c, h * stride[0] + xx, w + stride[1] + yy] * weight[co, c, xx, yy],
            reduce_type='sum'
        )
    )
    return Task(
