...
from hidet.ir.dialects.pattern import TaskPattern, any_const_ints, any_scalar_expr, int_vars, any_tensor_input, TensorAccessPattern
...
class Pattern:
    def __init__(self):
        ...
        self.x_access = TensorAccessPattern(indices_pattern=[self.an, self.ac, self.ap * 1 + self.arx, self.aq * 1 + self.ary])
        self.w_access = TensorAccessPattern(indices_pattern=[self.ac, self.arc, self.rx, self.ry])
        self.reduced_value = ReduceCompute(
            shape=[self.rc, self.rx, self.ry],
            axes=[self.arc, self.arx, self.ary],
            value=self.x_access * self.w_access,
            reduce_type='sum',
            data_type=None
        )
...
@register_impl('cuda_grid_static_conv2d_winograd_implementer')
class CudaGridStaticConv2dWinogradImplementer(Implementer):
    def __init__(self):
