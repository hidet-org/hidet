...
class CUDAFloat8e4m3MathFunctionSet(MathFunctionSet):
    """
    Math function set for float8_e4m3. Type casting for tanh(), erf(), and pow() is performed by the C++ implementation.
    """
    ...
    def call(self, name: str, *args) -> Expr:
        entry = primitive_func_pool.lookup_by_name(CUDA_FLOAT8E4M3 + name)
...
cuda_float8e4m3_math_function_set = CUDAFloat8e4m3MathFunctionSet()
cuda_float8e4m3_math_function_set.register()
register_math_function_set('cuda', 'float8_e4m3', cuda_float8e4m3_math_function_set)
