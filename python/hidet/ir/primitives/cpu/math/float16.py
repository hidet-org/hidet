...
class CPUFloat16MathFunctionSet(MathFunctionSet):
    @staticmethod
    def register():
        unary_funcs = {
            'sin': 'sin',
        ...
        binary_funcs = {'min': 'min', 'max': 'max', 'pow': 'pow', 'mod': 'mod', 'atan2': 'atan2'}
        ...
        for name_map, num_args in zip([unary_funcs, binary_funcs, ternary_funcs], [1, 2, 3]):
            for name, codegen_name in name_map.items():
                register_primitive_function(
                    name='cpu_f16_{}'.format(name),
                    codegen_name=codegen_name,
                    func_or_type=FuncType(
                        param_types=['float16'] * num_args,
                        ret_type='float16' if name not in ['isfinite', 'isinf', 'isnan'] else 'bool',
                    ),
                )
...
cpu_f16_math_function_set = CPUFloat16MathFunctionSet()
cpu_f16_math_function_set.register()
register_math_function_set('cpu', 'float16', cpu_f16_math_function_set)
