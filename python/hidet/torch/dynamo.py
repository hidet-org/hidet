...
def onnx2hidet(subgraph):
    ...
    example_outputs = onnx_module(*example_inputs)
    flow_graph: FlowGraph = hidet.trace_from(example_outputs, inputs=example_inputs)
    ...
    with hidet.option.search_space(search_space):
...
if success:
    # torch._dynamo is not available, skip
