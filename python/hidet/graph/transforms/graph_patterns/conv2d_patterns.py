...
class Conv2dScalePattern(SubgraphRewriteRule):
    ...
    def target(self, matched: MatchDict) -> Optional[List[Tensor]]:
        ...
        if not scale.shape[0] == scale.shape[2] == scale.shape[3] == 1:
            return None
        attrs = y.op.attrs
        return [ops.conv2d(x, w * scale.squeeze([0]).unsqueeze([3]), 
                           stride=attrs['stride'],
...
class TwoConv2dFusionPattern(SubgraphRewriteRule):
    ...
    def target(self, matched: MatchDict) -> Optional[List[Tensor]]:
        ...
        if op1.attrs['groups'] == op2.attrs['groups'] == 1:
            if same_list(op1.attrs['stride'], op2.attrs['stride']) and \
                same_list(w1.shape[1:], w2.shape[1:]) and \
                same_list(op1.attrs['dilations'], op2.attrs['dilations']) and \
                same_list(op1.attrs['padding'], op2.attrs['padding']) and \
                op1.attrs['pad_value'] == op2.attrs['pad_value']:
...
class ThreeConv2dFusionPattern(SubgraphRewriteRule):
    ...
    def target(self, matched: MatchDict) -> Optional[List[Tensor]]:
        ...
        if op1.attrs['groups'] == op2.attrs['groups'] == op3.attrs['groups'] == 1:
            if same_list(op1.attrs['stride'], op2.attrs['stride']) and same_list(op1.attrs['stride'], op3.attrs['stride']) and \
                same_list(w1.shape[1:], w2.shape[1:]) and  same_list(w1.shape[1:], w3.shape[1:]) and \
                same_list(op1.attrs['dilations'], op2.attrs['dilations']) and  same_list(op1.attrs['dilations'], op3.attrs['dilations']) and \
                same_list(op1.attrs['padding'], op2.attrs['padding']) and  same_list(op1.attrs['padding'], op3.attrs['padding']) and \
                op1.attrs['pad_value'] == op2.attrs['pad_value'] == op3.attrs['pad_value']:
