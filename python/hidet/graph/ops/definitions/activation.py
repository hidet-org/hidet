...
def hardswish(x: Tensor) -> Tensor:
    """
    The HardSwish operator is a variant of Swish that bounds the output between 0 and the input value.
...
def hardtanh(x: Tensor, min_val: float, max_val: float) -> Tensor:
    """
    The HardTanh operator is a variant of Tanh that bounds the output between a minimum and maximum value.
...
def softshrink(x: Tensor, lambda_val: float) -> Tensor:
    """
    The Softshrink operator is a switch function that is zero when the input is within a range,
    and otherwise equal to the input minus the lower bound or plus the upper bound.
    The range is defined by a lower bound of negative lambda and an upper bound of lambda.
    The output is the input minus lambda if the input is greater than lambda else input plus lambda.
    Parameters
    ----------
    x: Tensor
        The input tensor.
