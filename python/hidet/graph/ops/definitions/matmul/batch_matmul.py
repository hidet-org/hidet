...
from hidet.graph.ops.definitions.utils import tune, schedule_utils
...
class BatchMatmulTask(Task):
    ...
    def schedule_simt(
        ...
        used_smem_bytes_per_block = (
            block_shape[0] + block_shape[1]) * block_k * 2 * dtype.nbytes
        ...
        with hidet.script_module() as module:
            ...
            def copy_a_r2s(
                        regs_a_ldg: TensorType(dtype=dtype, layout=regs_a_ldg_layout),
                        smem_a: TensorType(dtype=dtype, layout=
            ...
            def copy_b_r2s(
                        regs_b_ldg: TensorType(dtype=dtype, layout=regs_b_ldg_layout),
                        smem_b: TensorType(dtype=dtype, layout=
            ...
            def mm_kernel(
                ...
                smem_a = tensor_pointer(dtype, layout=StridesLayout.from_shape(
                    [2, block_m, block_k], perm=[0, 2, 1]))
                smem_b = tensor_pointer(dtype, layout=StridesLayout.from_shape(
                ...
                regs_a_ldg = tensor('register', dtype,
                                    layout=regs_a_ldg_layout)
                regs_b_ldg = tensor('register', dtype,
