...
from hidet.ir.type import data_type, TensorType, TensorPointerType, tensor_type
from hidet.ir import primitives as prim
from hidet.ir.expr import convert, if_then_else
...
from hidet.lang import f16, f32, i32, u32, boolean, spatial, repeat, tensor, attr, grid, printf, cast, tensor_pointer, view, col_spatial
from hidet.lang.mapping import repeat, spatial
from hidet.lang.cuda import blockIdx, threadIdx, syncthreads, dynamic_shared_memory, register_tensor
from hidet.graph.ops.definitions.utils import Task, Operator, Tensor, TensorNode, InverseMap, compute, input_like, broadcast_shape, broadcast_shapes, broadcast_indices, can_mutually_broadcast
from hidet.utils.py import cdiv, is_power_of_two, prod
...
class AttnMaskAddTask(Task):
    def __init__(self, name: str, q: TensorNode, k: TensorNode, v: TensorNode, mask: TensorNode):
        ...
        o_head, q_head, k_head, v_head, mask_head = o_shape[:-2], q_shape[:-2], k_shape[:-2], v_shape[:-2], mask_shape[:-2]
        qk_head = broadcast_shape(q_head, k_head)
        batch_size = prod(o_head)
        qkv_dtype = q.type.dtype
        mask_shape = mask.const_shape()
        ...
        qk = compute(
            ...
            fcompute=lambda *indices: reduce(
                shape=[d_size],
                fcompute=lambda d:
                    q[broadcast_indices(indices[:-2], q_head, qk_head) + [indices[-2], d]] 
        ...
        max_value = compute(
            ...
            shape=reduced_shape,
            fcompute=lambda *indices: reduce(
                shape=[axis_extent], fcompute=lambda a: qk_masked[indices[:axis] + (a,) + indices[axis:]], reduce_type='max'
            ),
        )
        ...
        o = compute(
            ...
            fcompute=lambda *indices: reduce(
                shape=[n_size],
                fcompute=lambda a: 
                    sm[broadcast_indices(indices[:-2], qk_head, o_head) + [indices[-2], a]]
    ...
    def cuda_schedule_attn(self, i_split=2, block_i=16, block_j=256, block_size=128) -> IRModule:
        ...
        task = self
        node_q, node_k, node_v, node_mask, node_o = task.inputs[0], task.inputs[1], task.inputs[2],task.inputs[3], task.outputs[0]
        ...
        warp_count_m, warp_count_n, warp_count_k = block_i // warp_elems_m, block_j // warp_elems_n, cdiv(block_k, warp_elems_k)
        # Number of m16n8k16 mma's each warp performs in each dim. 1, 8, 4
        mmas_per_warp_m, mmas_per_warp_n, mmas_per_warp_k = warp_elems_m // mma_m, warp_elems_n // mma_n, warp_elems_k // mma_k
        ...
        warp_elems_m_o, warp_elems_n_o, warp_elems_k_o = block_i // warp_count_m_o, mma_n * cdiv(block_k// warp_count_n_o, mma_n), block_j // warp_count_k_o
        # Number of m16n8k16 mma's each warp performs in each dim. 1, 2, 16
        mmas_per_warp_m_o, mmas_per_warp_n_o, mmas_per_warp_k_o = warp_elems_m_o // mma_m, warp_elems_n_o // mma_n, warp_elems_k_o // mma_k
        ...
        smem_bytes_offsets = {
            ...
            'mij': smem_bytes_q + smem_bytes_k + smem_bytes_v + smem_bytes_qk + smem_bytes_l + smem_bytes_m + smem_bytes_lij,
        }

        dynamic_smem_bytes = smem_bytes_q + smem_bytes_k + smem_bytes_qk + smem_bytes_v + smem_bytes_l + smem_bytes_m + smem_bytes_lij + smem_bytes_mij
        ...
        smem_o_layout = (
            ...
            * row_major((block_i, swizzle_unit // 8)).swizzle(1) 
            * row_major((1, 8))
        )
        # smem_o_layout = row_major((block_i, block_k))

        smem_k_layout = (
            row_major((block_k // 8, block_j // 64)) 
            * row_major((8,8)).swizzle(1) 
            * row_major((1,8))
        )
        # smem_k_layout = row_major((block_k, block_j)) 
        smem_qk_layout = (
            row_major((block_i, block_j // 8)).swizzle(1) 
            * row_major((1,8))
        )
        ...
        # )
        smem_v_layout = (
            row_major((block_j // 8, block_k // 8)) 
            * row_major((8,1)).swizzle(1) 
            * row_major((1,8))
        )
        ...
        regs_li_new_layout = (
            row_major((rows_per_thread_mma_o, 1))
            * local_layout((block_i // rows_per_thread_mma_o, 1))
        )
        ...
        o_s2g_layout = spatial(cdiv(block_i, q_elems_per_thread), block_k) * repeat(q_elems_per_thread, 1) # 16 x 64, 8 elems per thread
        t_per_block_k_8_floor = block_size // (block_k // 8)
        if block_i < t_per_block_k_8_floor:
            q_g2s_layout = spatial(block_i, block_k // 8)
        else:
            q_g2s_layout = repeat(cdiv(block_i, t_per_block_k_8_floor), 1) * spatial(t_per_block_k_8_floor, block_k // 8)
        k_g2s_layout = repeat(cdiv(block_k, (block_size // (block_j // 8))), 1) * spatial(block_size // (block_j // 8), block_j // 8)
        ...
        with hidet.script_module() as module:
            ...
            def copy_k_g2s(
                ...
                for i, j_seg in k_g2s_layout.on(threadIdx.x):
                    ...
                    src_size = (
                        0
                        if (i >= d_size or offset_j + j >= n_size)
                        else min(n_size - j, 8)
                    )
            ...
            def copy_v_g2s(
                ...
                for i, j_seg in v_g2s_layout.on(threadIdx.x):
                    ...
                    src_size = (
                        0
                        if (offset_j + i >= n_size or j >= d_size)
                        else min(d_size - j, 8)
                    )
            ...
            def copy_q_g2s(
                ...
                for i, j_seg in q_g2s_layout.on(threadIdx.x):
                    ...
                    src_size = (
                        0
                        if (offset_i + i >= n_size or j >= d_size)
                        else min(d_size - j, 8)
                    )
            ...
            def copy_q_s2r(
                mma_i: int,
                k1: int,
                regs_q: f16[mma_config.a_elements],
                smem_q: smem_q_type
            ):
            ...
            def copy_k_s2r(
                mma_j: int,
                k1: int,
                regs_k: f16[mma_config.b_elements],
                smem_k: smem_k_type
            ):
                warp_id, lane_id = threadIdx.x / 32, threadIdx.x % 32
                for wi, wj, wk in spatial(warp_count_m, warp_count_n, warp_count_k).on(warp_id):
            ...
            def copy_qk_s2r(
                mma_i: int,
                k1: int,
                regs_qk: f16[mma_config.a_elements],
                ...
                for wi, wj, wk in spatial(warp_count_m_o, warp_count_n_o, warp_count_k_o).on(warp_id):
                    if not warp_id >= spatial(warp_count_m_o, warp_count_n_o, warp_count_k_o).num_workers:
                        p, q = col_spatial(16, 2).map(lane_id)
                        row_addr = ~smem_qk[wi * warp_elems_m_o + mma_i * mma_m + p, wk * warp_elems_k_o + k1 * mma_k + q * 8]
                        resolve_ldmatrix(regs_qk, row_addr, True)
                    # p, q = col_spatial(16, 2).map(lane_id)
                    # row_addr = ~smem_qk[wi * warp_elems_m_o + mma_i * mma_m + p, wk * warp_elems_k_o + k1 * mma_k + q * 8]
                    # b32_regs = view(regs_qk, u32[4])
                    # ldmatrix(
                    #     regs=[b32_regs[0], b32_regs[1], b32_regs[2], b32_regs[3]],
                    #     smem_addr=row_addr,
                    #     shared_space_addr=False,
            ...
            def copy_v_s2r(
                mma_j: int,
                k1: int,
                regs_v: f16[mma_config.b_elements],
                smem_v: smem_v_type
            ):
                warp_id, lane_id = threadIdx.x / 32, threadIdx.x % 32
                for wi, wj, wk in spatial(warp_count_m_o, warp_count_n_o, warp_count_k_o).on(warp_id):
            ...
            def attn_kernel(
                ...
                regs_k = tensor('register', dtype='float16', shape=[mmas_per_warp_k, mmas_per_warp_n, mma_config.b_elements])
                regs_acc = tensor('register', dtype=acc_dtype, shape=[mmas_per_warp_m, mmas_per_warp_n, mma_config.c_elements])
                regs_qk = tensor('register', dtype='float16', shape=[mmas_per_warp_m_o, mma_config.a_elements])
                regs_v = tensor('register', dtype='float16', shape=[mmas_per_warp_k_o, mmas_per_warp_n_o, mma_config.b_elements])
                regs_acc_o = tensor('register', dtype=acc_dtype, shape=[mmas_per_warp_m_o, mmas_per_warp_n_o, mma_config.c_elements])
                                ...
                                if wk == k_round:
                                    for mma_i, mma_j in grid(mmas_per_warp_m_o, 1):
                                        ...
                                        for i, j in c_store_map.on(lane_id):
                                            delta_m = wi * warp_elems_m_o + mma_i * mma_m + i
                            ...
                            for wi, wj, wk in spatial(warp_count_m_o, warp_count_n_o, warp_count_k_o).on(warp_id):
                                if wk == k_round:
                                    for mma_i, mma_j in grid(mmas_per_warp_m_o, mmas_per_warp_n_o):
                                        ...
                                        for i, j in mma_config.c_store_map.on(lane_id):
                                            delta_m = wi * warp_elems_m_o + mma_i * mma_m + i
                                            delta_n = wj * warp_elems_n_o + mma_j * mma_n + j
                                            smem_o[delta_m, delta_n] = (regs_mi_new[delta_m, 0] * smem_o[delta_m, delta_n] + regs_exp_mij[delta_m, 0] * regs_acc_o[mma_i, mma_j, p]) / regs_li_new[delta_m, 0]
...
class AttnMaskWhereOp(Operator):
    def __init__(self, q: Tensor, k: Tensor, v: Tensor, mask: Tensor):
        # super().__init__(inputs=[q, k, v, mask], task=AttnMaskWhereTask('attn_mask_where', input_like(
        #     q, 'q'), input_like(k, 'k'), input_like(v, 'v'), input_like(mask, 'mask')))
        raise NotImplementedError("Boolean type mask with Where operator not supported yet")

class AttnMaskAddOp(Operator):
    def __init__(self, q: Tensor, k: Tensor, v: Tensor, mask: Tensor):
        super().__init__(inputs=[q, k, v, mask], task=AttnMaskAddTask('attn_mask_add', input_like(
            q, 'q'), input_like(k, 'k'), input_like(v, 'v'), input_like(mask, 'mask')), attributes={})