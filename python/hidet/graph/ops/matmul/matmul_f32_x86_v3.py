...
class MatmulF32Taskx86_v2(Task):
    ...
    def schedule_matmulf32_x86(
        ...
        with hidet.script_module() as module:
            ...
            @hidet.script
            def matmul_kernel_x86_v2(a: float32[m_size, k_size], b: float32[k_size, n_size],
                ...
                for tidx in grid(nthreads, attrs=parallel_attr):
                    ...
                    # Before each loop, we compute the work id and comm id for the loop after it.
                    ...
                    my_end = -1
                    b_alg_loop5 = NC
                    thread_range_sub(loop5_nways, work_id_5th_loop, n_size, NR, ~my_start, ~my_end)
                    loop5_iter = my_start
                    while loop5_iter < my_end:
                        b_alg_loop5 = determine_blocksize_f_sub(loop5_iter, my_end, NC)
                        ...
                        # After getting the communicator and work id for the 3rd loop,
                        # we can now get the ids for the packing of B.
                        comm_id_packb = comm_id_3rd_loop
                        work_id_packb = comm_id_3rd_loop
                        packb_nways = loop3_nthreads

                        # Below: The start of loop4
                        b_alg_loop4 = KC
                        i_loop4 = 0
                        while i_loop4 < k_size:
                            ...
                            # Get the thread's partition of buffer and matrix
                            packed_b_buf = packb_buf + (
                                    packb_start_offsets[work_id_5th_loop] * packed_b_height)  # TODO: Check this
                            loop4_partition_b = b + (loop4_partition_b_start_row * n_size + loop4_partition_b_start_col)
                            ...
                            thrcomm_barrier(
                                ...
                                packb_nthreads  # TODO: Check this last parameter
                            )

                            # Start of the packing of B
                            for i_panel in range(npanels_b):
                                if i_panel % packb_nways != work_id_packb % packb_nways:
                                    continue
                                packed_b_buf_curr = packed_b_buf + (i_panel * packedb_panel_stride)
                                ...
                                if curr_panel_width == NR:
                                    ...
                                    for k_iter in range(k_iters):
                                        ...
                                        avx_f32x8_store_aligned(packed_b_buf_curr, b00)
                                        avx_f32x8_store_aligned(packed_b_buf_curr + 8, b08)
                                        packed_b_buf_curr += 16

                                        b10 = avx_f32x8_load(b_panel + n_size)
                                        b18 = avx_f32x8_load(b_panel + (n_size + 8))

                                        avx_f32x8_store_aligned(packed_b_buf_curr, b10)
                                        avx_f32x8_store_aligned(packed_b_buf_curr + 8, b18)
                                        packed_b_buf_curr += 16

                                        b20 = avx_f32x8_load(b_panel + (2 * n_size))
                                        b28 = avx_f32x8_load(b_panel + (2 * n_size + 8))

                                        avx_f32x8_store_aligned(packed_b_buf_curr, b20)
                                        avx_f32x8_store_aligned(packed_b_buf_curr + 8, b28)
                                        ...
                                        avx_f32x8_store_aligned(packed_b_buf_curr, b30)
                                        avx_f32x8_store_aligned(packed_b_buf_curr + 8, b38)
                                        ...
                                        avx_f32x8_store_aligned(packed_b_buf_curr, b40)
                                        avx_f32x8_store_aligned(packed_b_buf_curr + 8, b48)
                                        ...
                                        avx_f32x8_store_aligned(packed_b_buf_curr, b50)
                                        avx_f32x8_store_aligned(packed_b_buf_curr + 8, b58)
                                        ...
                                        avx_f32x8_store_aligned(packed_b_buf_curr, b60)
                                        avx_f32x8_store_aligned(packed_b_buf_curr + 8, b68)
                                        ...
                                        avx_f32x8_store_aligned(packed_b_buf_curr, b70)
                                        avx_f32x8_store_aligned(packed_b_buf_curr + 8, b78)

                                        packed_b_buf_curr += 16

                                    row += 8
                                    for remaining_row in range(k_remainder):
                                        b_panel = loop4_partition_b + (row * n_size + curr_panel_start)
                                        b00 = avx_f32x8_load(b_panel)
                                        b08 = avx_f32x8_load(b_panel + 8)
                            ...
                            # The barrier at the end of packing
                            ...
                            m_start_loop3 = 0
                            m_end_loop3 = 0
                            # Partition the dimension M for loop 3
                            thread_range_sub(
                                loop3_nways,
                                work_id_3rd_loop,
                                m_size,
                            ...
                            while ii < m_end_loop3:
                                ...
                                loop3_partition_c_start_row = ii
                                loop3_partition_a_start_row = ii

                                loop3_partition_a_start_col = loop4_partition_a_start_col
                                loop3_partition_b_start_col = loop4_partition_b_start_col
                                loop3_partition_c_start_col = loop4_partition_b_start_col

                                loop3_partition_height = b_alg_loop3
                                # TODO: Is this right?
                                loop3_partition_a_width = loop4_partition_b_height
                                loop3_partition_b_width = loop4_partition_b_width
                                loop3_partition_a_height = b_alg_loop3
                                loop3_partition_c_height = b_alg_loop3
                                ...
                                for ii_panel in range(npanels_a):
                                    ...
                                    if a_curr_panel_height == MR:  # we unroll the packing by 8
                                ...
                                # This marks the end of the packing of A, or so I wish
                                # Now let's go to the macrokernel
                                # But first, barrier...
                                thrcomm_barrier(
                                    comm_id_packa,
                                    ~packa_thrcomm_barrier_sense[work_id_3rd_loop],
                                ...
                                jr_nt = macro_nways
                                jr_tid = work_id_macro
                                ir_nt = loop1_nways
                                ir_tid = work_id_1st_loop
                                ...
                                macro_m = loop3_partition_a_height
                                macro_n = loop3_partition_b_width
                                macro_k = loop3_partition_a_width
                                ...
                                # Some variables as in the original code...
                                # TODO: There must be some useless ones, delete after passing tests
                                rs_packeda = 1
                                cs_packeda = MR
                                panel_dim_packeda = MR
                                ps_packed_a = packeda_panel_stride
                                rs_packedb = MR
                                cs_packedb = 1
                                ps_packed_b = packedb_panel_stride

                                rstep_a = ps_packed_a
                                cstep_b = ps_packed_b

                                rstep_c = rs_packeda * MR

                                cstep_c = NR
                                rstep_c = n_size * MR

                                macro_c_cast = as_tensor_pointer(
                                    ~c[loop3_partition_a_start_row, loop3_partition_b_start_col],
                                    dtype=float32,
                                    shape=(m_size, n_size)
                                )
                                ...
                                while j < jr_end:
                                    ...
                                    while i < ir_end:
                                        ...
                                        if m_cur == MR and n_cur == NR:
                                            ...
                                            if not is_first:
                                                for mm in range(m_cur):
                                                    for nn in range(n_cur):
                                ...
                                ii += b_alg_loop3
                            # End of loop4
                            # According to the original code, there seems to be a barrier here
                            # TODO: Looks weird, check later, especially about whether it's really the ids of packb
                            #  arrays that are used here
                            thrcomm_barrier(
                                comm_id_packb,
                                ~packb_thrcomm_barrier_sense[work_id_4th_loop],
                                ~packb_thrcomm_barrier_threads_arrived[work_id_4th_loop],
        ...
        assert isinstance(matmul_kernel_x86_v2, hidet.ir.Function)
        matmul_kernel_x86_v2.kind = "cpu_kernel"
