...
class MatmulF16CuteTask(Task):
    ...
    def __init__(self, a: TensorNode, b: TensorNode, parallel_k_parts: int = 1):
        ...
        super().__init__(
            name='matmul_f16_pk_cute', inputs=[a, b], outputs=[c], attributes={'parallel_k_parts': parallel_k_parts}
        )
    ...
    def schedule(
        ...
        with hidet.script_module() as module:
            ...
            from hidet.ir.cute.ops import tiled_tensor_view, rearrange, partition_src, partition_dst, copy
            ...
            def store_c_reg2gmem(
                regs_c: float16[mma_count_m, mma_count_n, mma_config.c_elements], c: float16[c_head + [m_size, n_size]]
            ):
            ...
            @hidet.script
            def store_c_smem2gmem(smem_c: float16[block_m, block_n], c: float16[c_head + [m_size, n_size]]):
                regs_c = register_tensor(float16, [store_c_layout.val_layout().size()])
...
class MatmulF16CuteOp(Operator):
    def __init__(self, a: Tensor, b: Tensor, parallel_k_parts=1):
        ...
        super().__init__(
            ...
            task=MatmulF16CuteTask(input_like(a, 'a'), input_like(b, 'b'), parallel_k_parts),
        )


def matmul_f16_cute(a: Tensor, b: Tensor, parallel_k_parts=1) -> Tensor:
    if len(a.shape) < 2 or len(b.shape) < 2:
    ...
    return MatmulF16CuteOp(a, b, parallel_k_parts).outputs[0]
