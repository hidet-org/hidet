        ...
        with hidet.script_module() as module:
            ...
            def softmax_cpu(x: float32[row_size, col_size], out: float32[row_size, col_size]):
                ...
                for i in grid(row_size, attrs=para):
                    ...
                    for j in range(col_size % 8):
                        max_val = max_val if max_val > x[i, col_size - col_size % 8 + j] else x[i, col_size - col_size % 8 + j]
                    ...
                    for j in range(col_size % 8):
                        out[i, col_size - col_size % 8 + j] = prim.exp(x[i, col_size - col_size % 8 + j] - max_val)
                        sum_value += out[i, col_size - col_size % 8 + j]
                    ...
                    for j in range(col_size % 8):
                        out[i, col_size - col_size % 8 + j] = out[i, col_size - col_size % 8 + j] / sum_value
