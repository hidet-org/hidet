...
from hidet.ir.primitives import cuda_max, cuda_sqrt
...
class BinaryElementwiseOp(Operator):
    def __init__(self, x: Tensor, y: Tensor, op, name: str):
        assert all(a == b for a, b in zip(x.shape, y.shape))
        super().__init__(inputs=[x, y], task=tasks.nn.binary_elementwise(name, x.shape, op=op))
...
def batch_norm_infer(x: Tensor, running_mean: Tensor, running_var: Tensor, epsilon=1e-5, axis=1):
    ...
    n, c, h, w = x.shape
    x - running_mean / (running_var + epsilon)
