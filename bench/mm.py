...
class MatMulTask(Task):
    ...
    def implement_cuda(self, workding_dir: str) -> IRModule:
        # return cuda_schedule_matmul_unrolled(self)
        return cuda_schedule_matmul(self)
        # return cuda_schedule_matmul_smem(self)

def cuda_schedule_matmul_unrolled(task: MatMulTask) -> IRModule:
    ...
    with hidet.script_module() as module:
        ...
        def mm_kernel(
            ...
            for i, k in a_s2r_layout.on(threadIdx.x):
                regs_a[0, i, k] = smem_a[0, i, 0]
            for k, j in b_s2r_layout.on(threadIdx.x):
                regs_b[0, k, j] = smem_b[0, 0, j]
            syncthreads()
            # Initialize regs C
            ...
            for k in range(k_tiles - 1):
                ...
                for k_frag in range(block_warps_k):
                    if k_frag == block_warps_k - 1:
                        ...
                        for i, k0 in a_g2s_layout.on(threadIdx.x):
                            smem_a[(k+1)%2, i, k0] = regs_a_ldg[i, k0]
                        for k0, j in b_g2s_layout.on(threadIdx.x):
                            smem_b[(k+1)%2, k0, j] = regs_b_ldg[k0, j]
                        syncthreads()
                        # Load next k-fragment (from next k-tile) from shared to local
                        for i, k0 in a_s2r_layout.on(threadIdx.x):
                            regs_a[(k_frag+1)%2, i, k0] = smem_a[(k+1)%2, i, 0]
                        for k0, j in b_s2r_layout.on(threadIdx.x):
                            regs_b[(k_frag+1)%2, k0, j] = smem_b[(k+1)%2, 0, j]
                    else:
                        # Load next k-fragment from shared to local
                        for i, k0 in a_s2r_layout.on(threadIdx.x):
                            regs_a[(k_frag+1)%2, i, k0] = smem_a[(k)%2, i, k_frag+1]
                        for k0, j in b_s2r_layout.on(threadIdx.x):
                            regs_b[(k_frag+1)%2, k0, j] = smem_b[(k)%2, k_frag+1, j]
                    if k_frag == 0:
                        # Load next AB tile from global into local
                        gmem_a = a[blockIdx.y, offset_m:, offset_k:]
                        for i, k0 in a_g2s_layout.on(threadIdx.x):
                            if offset_m + i < m_size:
                                regs_a_ldg[i, k0] = gmem_a.read([i,k0], protected=True)
                            else:
                                regs_a_ldg[i, k0] = 0.0
                        gmem_b = b[blockIdx.y, offset_k:, offset_n:]
                        for k0, j in b_g2s_layout.on(threadIdx.x):
                            if offset_n + j < n_size:
                                regs_b_ldg[k0, j] = gmem_b.read([k0,j], protected=True)
                            else:
                                regs_b_ldg[k0, j] = 0.0
                    # Perform MMA
                    for i, j in block_layout.on(threadIdx.x):
                        for k0 in range(warp_k):
                            regs_c[i, j] += regs_a[k_frag%2, i, k0] * regs_b[k_frag%2, k0, j]
            ...
            for k_frag in range(block_warps_k):
                if k_frag < block_warps_k - 1:
                    # Load next k-fragment from shared to local
                    for i, k0 in a_s2r_layout.on(threadIdx.x):
                        regs_a[(k_frag+1)%2, i, k0] = smem_a[(last_k)%2, i, k_frag+1]
                    for k0, j in b_s2r_layout.on(threadIdx.x):
                        regs_b[(k_frag+1)%2, k0, j] = smem_b[(last_k)%2, k_frag+1, j]
                # Perform MMA
                for i, j in block_layout.on(threadIdx.x):
                    for k0 in range(warp_k):
                        regs_c[i, j] += regs_a[k_frag%2, i, k0] * regs_b[k_frag%2, k0, j]
