...
def gemm_mma_fp16_kernel(bs, m_size, n_size, k_size) -> CompiledFunction:
    ...
    with hidet.script_module() as module:
        ...
        def gemm_mma_grid(a: f16[bs, m_size, k_size], b: f16[bs, k_size, n_size], c: f16[bs, m_size, n_size]):
            ...
            for i, j, p in repeat(4, 8, 2, mma_config.c_elements).on(0):
                regs_c[i, j, p] = 0.0
            ...
            for k0 in range((k_size + block_k - 1) // block_k):
                ...
                syncthreads()
                load_regs_a(warp_id, lane_id, smem_a, regs_a)
                load_regs_b(warp_id, lane_id, smem_b, regs_b)
                warp_mma(regs_a, regs_b, regs_c)
                syncthreads()
            store_c(warp_id, lane_id, regs_c, c)
...
if __name__ == '__main__':
    bs, m, n, k = 1, 128, 128, 128
    func = gemm_mma_fp16_kernel(bs, m, n, k)
    a = hidet.randn([bs, m, k], 'float16')
    b = hidet.randn([bs, k, n], 'float16')
    c = hidet.empty([bs, m, n], 'float16')
    func(a, b, c)
    print(a)
    print(b)
    print(c)
