...
def gemm_mma_fp16_all_kernel(batch_size, m_size, n_size, k_size) -> CompiledFunction:
    from hidet.lang import f16, f32, spatial, repeat, tensor, attr, printf, cast, col_spatial, view, u32, PointerType, tensor_pointer, grid, i32, void_p, var_of_function, static, \
        void
    ...
    from hidet.lang.cuda import MmaConfig, mma_sync, cp_async, cp_async_wait_all, cvta_generic_to_shared, ldmatrix, cp_async_wait_group, cp_async_commit_group, acquire_lock, \
        release_lock
    ...
    smem_bytes = max(2 * (block_m + block_n) * block_k, block_m * block_n) * 2
    ...
    with hidet.script_module() as module:
        ...
        @hidet.script
        def gemm_mma_fp16_all(
                num_args: int,
                arg_types: ~i32,
                args: ~void_p
        ):
            attr.func_kind = 'packed_func'
            attr.packed_func = var_of_function(gemm_mma_fp16_all_grid)
    ...
    func = hidet.driver.build_ir_module(ir_module, func_name='gemm_mma_fp16_all', func_type=hidet.ir.FuncType([~f32, ~f32, ~f32], void))
    return func
...
if __name__ == '__main__':
    ...
    func = gemm_mma_fp16_all_kernel(bs, m, n, k)
