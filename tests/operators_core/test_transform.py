...
def check_transform(shape, numpy_op, hidet_op, device, dtype=np.float32, atol=0, rtol=0):
    # wrap np.array(...) in case shape = []
    data = np.array(np.random.randn(*shape)).astype(dtype)
    numpy_result = numpy_op(data)
    hidet_result = hidet_op(hi.asarray(data).to(device=device)).cpu().numpy()
    np.testing.assert_allclose(actual=hidet_result, desired=numpy_result, atol=atol, rtol=rtol)


def check_transform_torch(shape, torch_op, hidet_op, device, dtype=np.float32, atol=0, rtol=0):
...
def test_reshape(shape, new_shape, device):
    check_transform(shape, lambda x: np.reshape(x, new_shape), lambda x: ops.reshape(x, new_shape), device)
...
def test_squeeze(shape, dims, device):
    check_transform(shape, lambda x: np.squeeze(x, axis=tuple(dims)), lambda x: ops.squeeze(x, dims), device=device)


@pytest.mark.parametrize("shape, dims", [[[3, 4], [0, 2]], [[2, 9, 9], [3]], [[], [0, 1, 2, 3]]])
def test_unsqueeze(shape, dims: List[int], device):
    check_transform(shape, lambda x: np.expand_dims(x, dims), lambda x: ops.unsqueeze(x, dims), device=device)
...
def test_flatten(shape, start_dim: int, end_dim: Optional[int], device):
    rank = len(shape)
    if start_dim < 0:
        start_dim += rank
    if end_dim < 0:
    ...
    check_transform(
        shape, lambda x: np.reshape(x, flattened_shape), lambda x: ops.flatten(x, start_dim, end_dim), device
    )
...
def test_transpose(shape, axes, device):
    check_transform(shape, lambda x: np.transpose(x, axes), lambda x: ops.transpose(x, axes), device)


@pytest.mark.parametrize("shape", [[33, 44], [1, 100], [100, 1], [10, 20], [20, 10], [100, 200], [2000, 3000]])
def test_transpose_2d(shape, device):
    check_transform(shape, lambda x: np.transpose(x), lambda x: ops.transpose(x), device)
...
def test_concat(shapes, dtype, axis, device):
    data_list = [np.random.randn(*shape).astype(dtype) for shape in shapes]
    numpy_result = np.concatenate(data_list, axis)
    hidet_result = ops.concat([hi.asarray(data).to(device=device) for data in data_list], axis).cpu().numpy()
...
def test_concat_empty(shapes, dtype, axis, device):
    torch_device = device_to_torch(device)
    data_list = [np.random.randn(*shape).astype(dtype) for shape in shapes]
    torch_result = torch.cat([torch.asarray(d).to(torch_device) for d in data_list], dim=axis)
    hidet_result = ops.concat([hi.asarray(data).to(device=device) for data in data_list], axis).cpu().numpy()
    np.testing.assert_allclose(actual=hidet_result, desired=torch_result.cpu().numpy(), atol=0, rtol=0)


@pytest.mark.parametrize("shape", [[33]])
@pytest.mark.parametrize('src_type', ['int8', 'int16', 'int32', 'int64', 'float16', 'float32', 'float64'])
@pytest.mark.parametrize('dst_type', ['int8', 'int16', 'int32', 'int64', 'float16', 'float32', 'float64'])
def test_cast(shape, src_type, dst_type, device):
    check_transform(shape, lambda x: x.astype(dst_type), lambda x: ops.cast(x, dst_type), device=device, dtype=src_type)


@pytest.mark.parametrize("shape, indices_shape, axis", [[[1234, 512], [128], 0], [[12, 34, 56], [2, 2], 1]])
def test_take(shape, indices_shape, axis, device):
    dim_extent = shape[axis]
    indices = np.random.randint(0, dim_extent - 1, indices_shape).astype(np.int64)
    check_transform(
        shape,
        lambda x: np.take(x, indices, axis),
        lambda x: ops.take(x, hi.asarray(indices).cuda(), axis),
        device=device,
    )


@pytest.mark.parametrize("shape, indices_shape, axis", [[[1234, 512], [2100, 512], 0], [[12, 34, 56], [12, 1, 56], 1]])
def test_gather(shape, indices_shape, axis, device):
    dim_extent = shape[axis]
    indices = np.random.randint(0, dim_extent - 1, indices_shape).astype(np.int64)
    check_transform_torch(
        shape,
...
def test_strided_slice(shape, starts, ends, axes, strides, device):
    ...
    check_transform(
        shape, lambda x: x[tuple(slice_obj)], lambda x: ops.strided_slice(x, starts, ends, axes, strides), device=device
    )
...
def test_broadcast(shape, broadcast_shape, device):
    check_transform(
        shape, lambda x: x + np.zeros(broadcast_shape), lambda x: ops.broadcast(x, broadcast_shape), device=device
    )
...
def test_getitem_nd(a_shape, b_shape, device):
...
def test_meshgrid(shapes, indexing, device):
    dtype = torch.float32
    atol = 0
    rtol = 0
    tensors = []
    ...
    tensors_hi = [hi.asarray(t).to(device=device) for t in tensors]
...
def test_getitem_nd_dynamic(a_shape, b_shape, device):
...
def test_getitem_advanced(a_shape, b_shape, c_shape, device):
    a = np.array(np.random.randn(*a_shape)).astype('float32')
...
def test_getitem_advanced_dynamic(a_shape, b_shape, c_shape, device):
    a_concrete_shape = [(i if isinstance(i, int) else i[1]) for i in a_shape]
    a_symbolic_shape = [(i if isinstance(i, int) else i[0]) for i in a_shape]
    ...
    a_hidet = asarray(a).to(device=device)
    b_hidet = asarray(b).to(device=device)
    c_hidet = asarray(c).to(device=device)
...
def test_adv_indexing_with_slices(device):
    ...
    x = hidet.randn(shape=(10, 11, 12, 13), dtype='float32', device=device)
    ind1 = hidet.randint(low=0, high=10, shape=(1, 1)).to(device=device)
    ind2 = hidet.randint(low=0, high=10, shape=(1, 2)).to(device=device)
...
def test_repeat_interleave(input_shape, repeats, dim, device):
    ...
    np.testing.assert_allclose(output_tensor.cpu().numpy(), output_tensor_hidet.cpu().numpy(), atol=0, rtol=0)
...
def test_unfold(shape, kernel_size, dilation, padding, stride, device):
    check_transform_torch(
        shape,
...
def test_as_strided(shape, size, stride, storage_offset, device):
    check_transform_torch(
        shape,
...
def test_flip(shape, dims, device):
    check_transform_torch(shape, lambda x: torch.flip(x, dims), lambda x: ops.flip(x, dims), device)
