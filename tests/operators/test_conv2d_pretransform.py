...
from hidet.graph.ops.conv2d.conv2d_gemm import pre_transform_img, pre_transform_imgv2, pre_transform_imgv3
from hidet.graph.ops.conv2d.conv2d_gemm import Conv2dGemmFp16PretransformV3Task
...
def test_pretransform_v3(img_dim, channel, padding, multi_8):
    img = hidet.randn([1, channel] + img_dim, device='cuda', dtype='float16')
    y1 = pre_transform_img(img, tuple(padding), 0.0, multi_8)
    y2 = pre_transform_imgv3(img, tuple(padding), 0.0, multi_8)
    assert torch.allclose(y1.torch(), y2.torch(), 1e-3, 1e-3)

    imgs = hidet.symbol([1, channel] + img_dim, dtype='float16', device='cuda')
    ys = pre_transform_imgv3(imgs, tuple(padding),0.0, multi_8)
