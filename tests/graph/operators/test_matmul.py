...
@pytest.mark.parametrize(
    ...
    [
        [[1, 128, 128], [1, 128, 128], "float32"],
        [[1, 333, 444], [1, 444, 555], "float32"],
    ]
)
@pytest.mark.parametrize(
    'mma',
...
@pytest.mark.parametrize(
    "a_shape, b_shape, dtype",
    [
        [[1, 128, 128], [128, 128], "float32"],
        [[333, 444], [444], "float32"],
    ]
)
def test_matmul(a_shape, b_shape, dtype):
    check_binary(a_shape, b_shape, lambda x, y: np.matmul(x, y), lambda x, y: ops.matmul(x, y), dtype=dtype, atol=1e-4, rtol=1e-4)
