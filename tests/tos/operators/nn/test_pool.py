...
def numpy_pool2d(data: np.ndarray, kernel: Tuple[int, int], stride: Tuple[int, int], padding: Tuple[int, int, int, int], reduce_type) -> np.ndarray:
            ...
            for p in range(oh):
                for q in range(ow):
                    if reduce_type == 'max':
                        ...
                        output[nn, cc, p, q] = np.sum(padded[nn, cc, p * sx: p * sx + kx, q * sy: q * sy + ky]) / (kx * ky)
...
def test_max_pool2d(shape, kernel, stride, padding):
    check_unary(shape, lambda x: numpy_pool2d(x, kernel, stride, padding, 'max'), lambda x: ops.max_pool2d(x, kernel, stride, padding), atol=0, rtol=0)
...
def test_avg_pool2d(shape, kernel, stride, padding):
    check_unary(shape, lambda x: numpy_pool2d(x, kernel, stride, padding, 'avg'), lambda x: ops.avg_pool2d(x, kernel, stride, padding), atol=1e-5, rtol=1e-5)
